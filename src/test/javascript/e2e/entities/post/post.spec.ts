...
describe('Post e2e test', () => {
  ...
  it('should load Posts', async () => {
    ...
    expect(await postComponentsPage.getTitle()).to.eq('bugTrackerJHipsterApp.post.home.title');
  });

  it('should load create Post page', async () => {
    await postComponentsPage.clickOnCreateButton();
    postUpdatePage = new PostUpdatePage();
    expect(await postUpdatePage.getPageTitle()).to.eq('bugTrackerJHipsterApp.post.home.createOrEditLabel');
  ...
  it('should delete last Post', async () => {
    ...
    expect(await postDeleteDialog.getDialogTitle()).to.eq('bugTrackerJHipsterApp.post.delete.question');
